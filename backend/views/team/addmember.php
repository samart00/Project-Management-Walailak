...
$this->title = 'เพิ่มผู้ใช้งานระบบในทีม';
...
$this->registerJsFile('@web/js/team/team-addmember.js',['depends' => [\yii\web\JqueryAsset::className()]]);
		...
		var formData = new FormData();
	        ...
	        formData.append('data',JSON.stringify(listUser));
	  	    formData.append('id', "$id");
		        ...
		        request.open("POST", "$baseUrl/team/addmember", false);
		        			...
		        			if(response.isDelete){
	        					...
	        					if(response.isClose){
		        					...
		        					if(response.success){
				        				$('#success').modal('show');
										setTimeout(function(){
							            	$.redirect('$baseUrl/team/member', {'$csrfParam':'$csrf','teamId':'$teamId'});
										}, 2000); 
		        					}
								}
							}
...
<div class="box box-solid">
	<div class="box-header with-border">
		<div class="row">
			<?php $form = ActiveForm::begin(['action'=>$baseUrl.'/team/add']); ?>
			...
			<form action="<?=$baseUrl."/team/member"?>" method="post" style="display: inline;">
						...
						<input type="hidden" name="<?=$csrfParam ?>" value="<?=$csrf?>">
	           	 		<a  href="javascript:;" onclick="this.parentNode.submit()" type="button" class="btn btn-default" aria-label="Left Align" title="ย้อนกลับ"> 
	                	  <span class="" aria-hidden="true">ย้อนกลับ</span>
	                	</a>
                </form>
				
				<button id="addMember" type="button" class="btn btn-success" aria-label="Left Align" title="เพิ่มสมาชิก">
...
<div class="panel" style="padding: 10px;">
	<?php $form = ActiveForm::begin(['method'=>'get', 'action' => $baseUrl.'/team/add']); ?>
		<div class="row">
			<input type="hidden" name="teamId" value="<?=$id?>">
